...
void glWidget::mouseMoveEvent(QMouseEvent* event)
{
    ...
    float y = -((float)clickPos.y() / (float)this->height()) * 2.0 + 1.0;
    if (mouseDragging) {
        float factor = 100 / this->height();
        float dx = factor * (x - mouseLastX);
        float dy = factor * (y - mouseLastX);
    ...
    mouseLastX = x;
    mouseLastY = y;
    modelMatrix1.rotate(-angleY, QVector3D(1.0f, 0.0f, 0.0f)); // rotate(degree, axis)
                               ...
                               QMatrix4x4 projMatrix, QMatrix4x4 viewMatrix, QMatrix4x4 modelMatrixTriangleSet1, QMatrix4x4 modelMatrixTriangleSet2)
{
    ...
    transformMatrix = projMatrix;
    transformMatrix *= viewMatrix;
    transformMatrix *= modelMatrixTriangleSet1;
    ...
    if (modelMatrixTriangleSet2.data() != 0 ) {
        transformMatrix = projMatrix;
        transformMatrix *= viewMatrix;
        transformMatrix *= modelMatrixTriangleSet2;
        initAttributeVariable(aPosition, 3, vertexBuffer);
        initAttributeVariable(aColor, 3, colorBuffer);
        glUniformMatrix4fv(uMVPMatrix, 1, GL_FALSE, transformMatrix.data()); //pass current model-view-projection Matrix to shader
        glDrawArrays(GL_TRIANGLES, 0, numVertices);
